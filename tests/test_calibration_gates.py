...
def test_calibration_gates_pdf_uses_absolute_gap_with_explicit_note():
    failures, notes = task_gate_failures(
    ...
    failing, _ = task_gate_failures(_gate(fmt="pdf", flip_rate=None, shuffled=Fraction(7, 2)))
    assert any("shuffled" in f for f in failing)
