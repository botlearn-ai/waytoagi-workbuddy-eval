...
tasks/
  ...
  豆包工作/
    t_01/
      ...
      prompt.txt               完整原始题面
      reference_files/         本题参考附件，原始文件名直接放在此层
...
参考附件在题包中使用 `reference_files/原始文件名`，工具可按题面中的文件名直接查找。下载缓存位于 `data/references/<URL 摘要>/`，三个产品各自获得独立副本。每个题包包含题面和参考附件，评分条件和专家答案由判分端读取。下载失败可重跑；完整附件复用本地缓存。

重复运行 prepare 会将已展开的嵌套附件整理到 reference_files 的第一层，核对文件内容后清理对应的空子目录，并同步 manifest.json。已有答案与人工记录保留；附件同名或输入被改动时停止并提示核对。
